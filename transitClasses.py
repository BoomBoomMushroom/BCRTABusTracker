...
class Feed:
    def __init__(self, feedObj: gtfs_realtime_pb2.FeedMessage):
        self.timestamp = feedObj.header.timestamp
...
class VehicleEntity:
    def __init__(self, vehicleObj):
        ...
        self.timestamp: int = vehicleObj.vehicle.timestamp # not sure, maybe from when it last pinged?
...
class Trip:
    ...
    def getRoute(self) -> Route: return Route.getRouteFromId(self.routeId)
    def getShape(self) -> None: return None # TODO: make it return the shape and a list of points for that or smthn
